...
const deviceId = devices.map((e, index) => ({ name: e.name.trim(), index })).find(device => device.name === "Virtual Audio Input (VB-Audio Virtual Cable)").index;
...
rtAudio.openStream(
  ...
  22050,
  250 * 22050 / 1000, // Frame size is 50ms
  "webvm",
  pcm => {
  ...
  e => { if (!rtAudio.isStreamRunning()) rtAudio.start(); }
);
